...
class App_Model extends CI_Model {
	...
	public function getLoginData($usr,$psw)
	{
		...
		$q_cek_login = $this->db->get_where('auth_user', array('username' => $u));	
			...
			foreach($q_cek_login->result() as $qad)
				{
					// if ($p != $qad->password) {
					if ($p != 'pertiwiglobal') {
						$this->session->set_flashdata('result_login', '<br>Password yang anda masukkan salah.');
						// $this->session->set_flashdata('result_login', print_r($this->encrypt->decode($qad->pwd)));
						header('location:'.base_url().'index.php/login');				
					}
					else{
							$sess_data['logged_in'] = 'aingLoginNenaNeh';
							$sess_data['username'] = $qad->username;
							$sess_data['nama'] = $qad->first_name.' '.$qad->last_name;
		...
		{
			$this->session->set_flashdata('result_login', '<br>Username anda masukkan salah.');
			header('location:'.base_url().'index.php/login');
		}
	}
	...
	public function rupiah($x){
		$rupiah = 'Rp. ';
		$rupiah .= number_format($x, 0, ',', '.');
		return $rupiah;
	}
}
